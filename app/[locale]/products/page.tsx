...
import { localizedProductName } from "@/lib/product-name-i18n";
...
function ProductViewer({
  ...
  const categoryLabel = t(`categories.${product.categoryId}.label`);
  const displayName = localizedProductName(product.file, locale, product.name);
...
const ProductCard = memo(function ProductCard({
  ...
  const badgeLabel =
    showSubcategory && product.subcategoryId
      ? t(`subcategories.${product.subcategoryId}`)
      : categoryLabel;
  const imageSrc = productImageUrl(product.storageFolder, product.file);
    ...
    <article className="group relative flex flex-col justify-between border border-iba-sky/10 bg-white p-1.5 sm:p-3 lg:p-6 shadow-sm shadow-iba-sky/[0.04] transition-[transform,box-shadow,border-color] duration-200 hover:-translate-y-0.5 hover:border-iba-navy/40 hover:shadow-[0_8px_28px_rgba(0,170,226,0.1)]">
      ...
      <div className="group/img relative mb-2 sm:mb-4 lg:mb-6 aspect-square w-full overflow-hidden rounded-md sm:rounded-lg border border-iba-sky/5 bg-iba-navy/[0.06] transition-colors group-hover:bg-iba-navy/10">
        ...
        <button
          ...
          aria-label={t("enlargeImage", { name: displayName })}
        >
        ...
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <motion.div {...fade} className="max-w-4xl">
            ...
            <h1 className="mt-4 text-4xl font-black uppercase leading-[0.95] tracking-tighter text-white sm:text-6xl md:text-8xl">
              {t("heroTitle")}{" "}
              <span className="text-iba-sky">{t("heroTitleAccent")}</span>
            </h1>
            ...
            <div ref={productsTopRef} className="min-w-0 flex-1 scroll-mt-28">
              ...
              {filteredProducts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16 text-center lg:py-24">
                  ...
                  <p className="text-lg sm:text-xl font-bold uppercase text-iba-sky">{t("noProducts")}</p>
                  <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-iba-sky/60">{t("noProductsHint")}</p>
                </div>
              ) : null}
            </div>
          </div>
