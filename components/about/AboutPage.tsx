...
const focusCardIds = ["impact", "projects", "governance"] as const;
...
export function AboutPage() {
  ...
  const jsonLd = {
    ...
    "@type": "VideoObject",
    name: `${tMeta("brandFull")} - ${t("mission.badge")}`,
    description: t("mission.intro"),
    uploadDate: "2026-07-01T08:00:00+01:00",
    embedUrl: ABOUT_VISION_VIMEO_EMBED_URL,
    thumbnailUrl: ["https://www.afromosia-rdc.com/images/iba-video-thumbnail.jpg"],
  };
  ...
  return (
    <main className="flex min-h-[100dvh] md:min-h-screen flex-col bg-background selection:bg-iba-navy selection:text-white">
      ...
      <section className="relative overflow-hidden bg-iba-navy py-20 text-white md:py-28 lg:py-32">
        <div
          className="pointer-events-none absolute -right-32 top-0 h-[600px] w-[600px] rounded-full bg-white/[0.04] blur-[100px]"
          aria-hidden
        />
        <div
          className="pointer-events-none absolute -left-32 bottom-0 h-[400px] w-[400px] rounded-full bg-iba-sky/[0.04] blur-[100px]"
          aria-hidden
        />
        ...
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <AboutSectionHeader
            variant="navy"
            waterNumber={t("mission.waterNumber")}
            badge={t("mission.badge")}
            kicker={t("mission.kicker")}
            title={
              <>
                {t("mission.headline")}{" "}
                <span className="text-iba-sky italic">{t("mission.headlineAccent")}</span>
              </>
            }
            lead={t("mission.intro")}
          />
          ...
          >
            <div className="lg:col-span-8 space-y-6 text-base font-medium leading-relaxed text-white/90 md:text-lg">
              <h3 className="text-2xl font-black uppercase tracking-tight text-white md:text-3xl border-b border-white/10 pb-4">
                {t("mission.narrativeTitle")}
              </h3>
              <p className="text-xl font-light text-white/95">{t("mission.narrativeLead")}</p>
              <p className="text-white/80">{t("mission.narrativeBody")}</p>
            </div>
      ...
      <section className="relative bg-background py-20 md:py-32">
        <div className="mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20 text-center flex flex-col items-center">
          <span className="mb-6 inline-block font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-iba-navy">
            {t("vision.badge")} // {t("vision.kicker")}
          </span>
          <motion.h2
            {...fade}
          ...
          >
            ...
            <span className="relative inline-block text-iba-navy">
              {t("vision.headlineAccent")}
              <svg
                className="absolute -bottom-2 left-0 w-full h-3 text-iba-navy/30"
          ...
          <motion.p
            ...
            className="mt-8 max-w-3xl text-lg font-medium leading-relaxed text-iba-muted md:text-xl"
          >
            {t("vision.intro")}
          </motion.p>
        </div>
      ...
      <section className="relative overflow-x-clip bg-iba-navy py-20 text-white md:py-28 lg:py-32">
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <AboutSectionHeader
            variant="navy"
            waterNumber={t("focusAreas.waterNumber")}
            badge={t("focusAreas.badge")}
            kicker={t("focusAreas.kicker")}
            title={
              <>
                {t("focusAreas.headline")}{" "}
                <span className="text-iba-sky">{t("focusAreas.headlineAccent")}</span>
              </>
            }
            lead={t("focusAreas.intro")}
          />

          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {focusCardIds.map((id, i) => (
              <motion.article
                key={id}
                {...fade}
              ...
              >
                ...
                {id === "projects" ? (
                  ...
                  >
                    {t("focusAreas.cards.projects.ctaLabel")}
