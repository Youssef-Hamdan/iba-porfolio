...
export function AboutPage() {
  ...
  return (
    <main className="flex min-h-screen flex-col bg-background selection:bg-iba-sky selection:text-white">
        ...
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <motion.div {...fade} className="max-w-4xl">
            <div className="mb-6 flex items-center gap-4">
              <span className="h-px w-12 bg-iba-sky" />
              <p className="font-mono text-xs font-bold uppercase tracking-[0.2em] text-iba-navy/50">
                International Business Alliance
              </p>
            </div>
            <h1
              id="about-page-heading"
              className="mt-3 text-5xl font-black uppercase leading-[0.95] tracking-tighter text-iba-navy sm:text-6xl md:text-8xl"
            >
              {pageTitle}
            </h1>
            <p className="mt-8 max-w-2xl text-lg font-medium leading-relaxed text-iba-navy/70 md:text-xl border-l-2 border-iba-sky pl-6">
              {pageIntro}
            </p>
          </motion.div>
        </div>
      ...
      {/* 2. MISSION — BANDEAU SKY (Structural Layout) */}
      <section className="relative overflow-hidden bg-iba-sky py-20 text-white md:py-28 lg:py-32">
        ...
        <div className="pointer-events-none absolute -left-32 bottom-0 h-[400px] w-[400px] rounded-full bg-iba-navy/[0.04] blur-[100px]" aria-hidden />
        ...
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <AboutSectionHeader
            ...
            title={
              <>
                {mission.headline}{" "}
                <span className="text-iba-navy italic">{mission.headlineAccent}</span>
              </>
            }
            lead={mission.intro}
          />
          ...
          >
            ...
            <aside className="group relative lg:col-span-4 overflow-hidden rounded-sm bg-iba-navy p-8 shadow-2xl lg:sticky lg:top-32 transition-transform hover:-translate-y-1">
              {/* Top heavy steel beam accent */}
              <div className="absolute top-0 left-0 h-1.5 w-full bg-iba-blue transition-all duration-500 group-hover:bg-white" />
              
              <div className="relative z-10">
                <ArrowDownRight className="mb-6 h-8 w-8 text-iba-sky opacity-50" />
                <p className="font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-iba-sky">
      ...
      <section className="relative bg-background py-20 md:py-32">
        <div className="mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20 text-center flex flex-col items-center">
          <span className="mb-6 inline-block font-mono text-[10px] font-bold uppercase tracking-[0.2em] text-iba-sky">
            {vision.badge} // {vision.kicker}
          </span>
          <motion.h2 
            {...fade}
            className="text-balance text-4xl font-black uppercase leading-[1.05] tracking-tighter text-iba-navy md:text-6xl lg:text-7xl max-w-5xl"
          >
            {vision.headline}{" "}
            <span className="relative inline-block text-iba-sky">
              {vision.headlineAccent}
              {/* Decorative underline */}
              <svg className="absolute -bottom-2 left-0 w-full h-3 text-iba-sky/30" preserveAspectRatio="none" viewBox="0 0 100 100" fill="none">
                <path d="M0,50 L100,50" stroke="currentColor" strokeWidth="6" />
              </svg>
            </span>
          </motion.h2>
          <motion.p {...fade} transition={{ delay: 0.1 }} className="mt-8 max-w-3xl text-lg font-medium leading-relaxed text-iba-navy/70 md:text-xl">
            {vision.intro}
          </motion.p>
        </div>
        <SectionWave />
      ...
      {/* 4. AXES — SKY (Industrial Pillar Cards) */}
      <section className="relative overflow-x-clip bg-iba-sky py-20 text-white md:py-28 lg:py-32">
        <div className="relative z-10 mx-auto max-w-[90rem] px-5 sm:px-8 md:px-16 lg:px-20">
          <AboutSectionHeader
            variant="sky"
            ...
            title={
              <>
                {focusAreas.headline}{" "}
                <span className="text-iba-navy">{focusAreas.headlineAccent}</span>
              </>
            }
          ...
          <div className="mt-12 grid gap-5 md:grid-cols-3">
            {focusAreas.cards.map((card, i) => (
              ...
              >
                ...
                <div className="absolute left-0 top-0 h-full w-1.5 bg-iba-navy transition-all duration-500 group-hover:w-2 group-hover:bg-white" />
                ...
                {"ctaHref" in card && card.ctaHref ? (
                  <Link
                    href={card.ctaHref}
                    className="mt-8 inline-flex w-fit items-center border-b border-transparent pb-1 font-mono text-[10px] font-bold uppercase tracking-widest text-iba-navy transition-all hover:border-white hover:text-white"
                  >
