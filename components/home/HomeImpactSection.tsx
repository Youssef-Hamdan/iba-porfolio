...
const pillarKeys = ["innovation", "supply", "trade"] as const;

const pillarImages: Record<(typeof pillarKeys)[number], string[]> = {
  innovation: ["/images/pillar1.webp", "/images/pillar2.webp"],
  supply: [
...
export function HomeImpactSection() {
  const t = useTranslations("HomeImpact");
  ...
  const parallaxBlocks = pillarKeys.map((key) => ({
    images: pillarImages[key],
    title: t(`pillars.${key}.title`),
    titleNode: (
      <>
        {t(`pillars.${key}.titleBefore`)}{" "}
        <span className="text-iba-sky">{t(`pillars.${key}.titleAccent`)}</span>
      </>
    ),
    desc: t(`pillars.${key}.description`),
  }));
    ...
    <section className="relative z-20 w-full overflow-hidden bg-background">
      <div className="relative bg-iba-navy py-12 text-white md:py-16">
        ...
        <div className="relative z-10 mx-auto w-full max-w-[90rem] px-4 sm:px-8 md:px-16 lg:px-20">
          ...
          >
            <StatCard icon={<Handshake />} numericValue={5} suffix="" label={t("stats.partners")} />
            <StatCard icon={<Store />} numericValue={70} suffix="+" label={t("stats.stores")} />
            <StatCard icon={<Target />} numericValue={25} suffix="+" label={t("stats.projects")} />
            <StatCard icon={<Users />} numericValue={40} suffix="+" label={t("stats.employees")} />
          </motion.div>
        </div>
